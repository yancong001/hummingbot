...
def supertrend(data, period=13, atr_multiplier=3.):
    ...
    # Compute SuperTrend
    data['SuperTrend'] = (data['high'] + data['low']) / 2
    data['SuperTrend'] = data['SuperTrend'].shift(1)
    data['SuperTrend'] = data['SuperTrend'].where(data['atr'] >
                                                  data['SuperTrend'].abs(), data['SuperTrend'] - data['atr'])
    data['SuperTrend'] = data['SuperTrend'].where(data['atr'] >
                                                  data['SuperTrend'].abs(), data['SuperTrend'] + data['atr'])

    # Compute upper and lower bands
    data['upperband'] = data['SuperTrend'] + data['atr']
    data['lowerband'] = data['SuperTrend'] - data['atr']
...
class AutoRebalance(ScriptStrategyBase):
    ...
    def on_tick(self):
        ...
        if self.uptrend == "True" and self.downtrend == "False":
            self.hold_asset_config = {"BUSD": Decimal('10.00')}
            self.coin_config = {
                "BTC": Decimal('30.00'),
                "ETH": Decimal('20.00'),
                "BNB": Decimal('20.00'),
            ...
            self.hold_asset_config = {"BUSD": Decimal('60.00')}
            self.coin_config = {
                "BTC": Decimal('10.00'),
                "ETH": Decimal('10.00'),
                "BNB": Decimal('10.00'),
            ...
            self.hold_asset_config = {"BUSD": Decimal('30.00')}
            self.coin_config = {
                "BTC": Decimal('20.00'),
                "ETH": Decimal('15.00'),
                "BNB": Decimal('15.00'),
    ...
    def format_status(self) -> str:
        ...
        lines.extend([f"  Short period ATR:   {self.atr}"] +
                     [f"  Long period ATR:    {self.l_atr}"] +
                     [f"  Is volatile: {self.is_volatile}\n"] +
                     [f"  SuperTrend upperband: {np.round(self.upperband, 4)}"] +
                     [
                         f"  SuperTrend current:   {Decimal(self.connector.get_mid_price('BTC-BUSD')).quantize(Decimal('1.0000'))}"] +

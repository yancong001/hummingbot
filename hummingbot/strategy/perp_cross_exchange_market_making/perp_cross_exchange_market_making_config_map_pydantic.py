...
from hummingbot.client.config.config_validators import validate_bool, validate_connector, validate_derivative
        ...
        if field.name == "taker_market":
            ...
            cls.__fields__["taker_market"].type_ = ClientConfigEnum(  # rebuild the exchanges enum
                ...
                names={e: e for e in sorted(
                    AllConnectorSettings.get_derivative_names().union(
                        AllConnectorSettings.get_gateway_amm_connector_names()
                    ))},
                type=str,
            )
